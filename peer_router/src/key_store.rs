...
pub const SIGNED_DATA_HEADER_SIZE: usize = 2 * SIGNATURE_SIZE + VERIFY_KEY_SIZE;
...
impl KeyStore {
    pub fn new(
        key_pair: &[u8; SIGNING_KEY_SIZE],
        sig: &[u8; SIGNATURE_SIZE],
        ca: &[u8; VERIFY_KEY_SIZE],
    ...
    pub fn verify<'a, 'b>(
        &self,
        data: &'a [u8],
        sig: &'b [u8; SIGNED_DATA_HEADER_SIZE],
        ...
        self.ca
            .verify(verifying_key, &key_sig)
            .or(Err(KeyStoreError::VerifyError))?;

        let verifying_key =
            VerifyingKey::try_from(verifying_key).or(Err(KeyStoreError::MalformedDataHeader))?;
        let data_sig = Signature::try_from(data_sig).or(Err(KeyStoreError::MalformedDataHeader))?;
        verifying_key
            .verify(data, &data_sig)
            .or(Err(KeyStoreError::VerifyError))?;
        Ok(())
    }
}
